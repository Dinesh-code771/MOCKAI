...
export class CronProcessor extends WorkerHost {
  ...
  async process(
    ...
    try {
      ...
      this.logger.error(logString_, error?.stack, 'CronProcessor');
      if (typeof job.log === 'function') job.log(logString_);
      throw error;
    }
  }
  ...
  async onProgress(job: Job) {
    ...
    if (typeof job.log === 'function')
      job.log(`Job ${job.id} is ${job.progress}% complete`);
  }
  ...
  async onCompleted(job: Job) {
    ...
    if (typeof job.log === 'function')
      job.log(`Job ${job.id} has been completed`);
  }
  ...
  async onStalled(job: Job) {
    ...
    if (typeof job.log === 'function')
      job.log(`Job ${job.id} has been stalled`);
